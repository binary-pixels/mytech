...
export async function GET(request: Request) {
  ...
  if (isExport) {
    ...
    for (const entry of index) {
      ...
      if (session) {
        // Collect all sessions with same email
        ...
        for (const sib of siblings) {
          ...
          const sibSession = readSession(sib.id);
          if (sibSession) {
  ...
  // List mode — read from index (metadata only, no messages)
  let merged = mergeSessions(readIndex());
...
export async function POST(request: Request) {
  try {
    ...
    // If same email already has a session, return it (resume chat)
    if (body.email) {
      const existing = index.find((s) => s.customerEmail === body.email);
      if (existing) {
        const session = readSession(existing.id);
        if (session) {
